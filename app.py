...
def norm_col(x) -> str:
    s = txt(x).lower()
    s = s.translate(str.maketrans("çğıöşüıİ", "cgiosuii"))
...
def norm_tr(x) -> str:
    ...
    return f"TR{int(m.group(1))}" if m else s
...
def filter_scada_data_errors(scada_with_cbs: pd.DataFrame, use_single_spike_filter: bool):
...
def build_analysis(cbs: pd.DataFrame, fifteen_min: pd.DataFrame, new_request_kva: float):
    ...
    analysis["bos_kapasite_kva"] = analysis["kurulu_guc_kva"] - analysis["karar_demand_kva"]

    request = float(new_request_kva)
    analysis["yeni_talep_kva"] = request
    analysis["yeni_talep_sonrasi_demand_kva"] = analysis["karar_demand_kva"] + request
...
def recommend_transfer_candidates(
    analysis: pd.DataFrame,
    selected_key,
    source_target_pct: float,
    candidate_upper_pct: float,
    max_candidates: int,
) -> pd.DataFrame:
    selected = analysis[
    ...
    mahalle = txt(source["mahalle"])

    source_capacity = float(source["kurulu_guc_kva"])
    source_current_demand = float(source["karar_demand_kva"])
    source_current_pct = float(source["yuklenme_orani_pct"])
    source_target_demand = source_capacity * float(source_target_pct) / 100
    needed_transfer_kva = max(0.0, source_current_demand - source_target_demand)

    candidates = analysis.copy()
    candidates = candidates[
        candidates["mahalle"].astype(str).eq(mahalle)
        & ~(
            (candidates["montaj_key"].eq(source["montaj_key"]))
            & (candidates["tr_code"].eq(source["tr_code"]))
        )
        & candidates["karar_demand_kva"].notna()
        & candidates["yuklenme_orani_pct"].notna()
        & (candidates["yuklenme_orani_pct"] <= float(candidate_upper_pct))
    ...
    candidates["aday_limit_demand_kva"] = candidates["kurulu_guc_kva"] * float(candidate_upper_pct) / 100
    candidates["aday_alabilecegi_yuk_kva"] = (
        candidates["aday_limit_demand_kva"] - candidates["karar_demand_kva"]
    ).clip(lower=0)

    candidates = candidates[candidates["aday_alabilecegi_yuk_kva"] > 0].copy()

    if candidates.empty:
        return candidates

    candidates["onerilen_aktarim_kva"] = candidates["aday_alabilecegi_yuk_kva"].clip(upper=needed_transfer_kva)
    candidates["kaynak_aktarim_sonrasi_demand_kva"] = source_current_demand - candidates["onerilen_aktarim_kva"]
    candidates["kaynak_aktarim_sonrasi_yuklenme_pct"] = (
        candidates["kaynak_aktarim_sonrasi_demand_kva"] / source_capacity * 100
    )
    candidates["aktarim_sonrasi_demand_kva"] = candidates["karar_demand_kva"] + candidates["onerilen_aktarim_kva"]
    ...
    candidates["kaynak_mevcut_yuklenme_pct"] = source_current_pct
    candidates["kaynak_hedef_yuklenme_pct"] = float(source_target_pct)
    candidates["kaynak_ihtiyac_aktarim_kva"] = needed_transfer_kva

    return candidates.sort_values(
        ["onerilen_aktarim_kva", "yuklenme_orani_pct"],
...
def main():
    ...
    st.caption(f"Sürüm: {APP_VERSION} | Haritasız stabil sürüm. Algoritmik pik ilk mantığa döndürüldü.")
    ...
    with st.sidebar:
        ...
        st.divider()
        st.header("2) Veri Temizleme")
        only_valid = st.toggle("Sadece Valid kalite", value=True)
        remove_zeros = st.toggle("0 demand değerlerini kaldır", value=True)
        use_single_spike_filter = st.toggle(
            "Tekil sıçrama filtresini uygula",
            value=False,
            help="Açılırsa tek ölçümlük aşırı sıçramaları filtreler. Default kapalıdır çünkü piki fazla bastırabilir.",
        )

        h_mode_label = st.radio(
            "Aynı T kodunda birden fazla H varsa",
            ["H hücrelerini topla", "H hücrelerinden en büyüğünü al"],
            index=0,
        )
        h_mode = "sum" if h_mode_label == "H hücrelerini topla" else "max"

        st.divider()
        st.header("3) Risk / Öneri Ayarları")
        min_source_load_pct = st.slider("Öneri üretilecek min. trafo yüklenmesi (%)", 60, 160, 105, 5)
        source_target_pct = st.slider("Riskli trafo hedef yüklenmesi (%)", 40, 100, 70, 5)
        candidate_upper_pct = st.slider("Aday trafo üst yük sınırı (%)", 30, 100, 55, 5)
        max_candidates = st.slider("Riskli trafo başına aday sayısı", 1, 10, 3, 1)

        st.divider()
        st.header("4) Yeni Bağlantı")
        new_request_kva = st.number_input("Yeni talep (kVA)", min_value=1.0, value=50.0, step=10.0)

        st.divider()
        st.header("Algoritma Notu")
        st.caption("Karar Demandı = Algoritmik Pik Demand")
        st.caption("Algoritmik Pik: 15 dk veride kısa pencere skoru")
        st.caption("Yüklenme = Karar Demandı / Trafo Gücü × 100")
    ...
    try:
        ...
        scada_clean, scada_errors = filter_scada_data_errors(
            scada_with_cbs,
            use_single_spike_filter=use_single_spike_filter,
        )

        fifteen_min = aggregate_scada(scada_clean, h_mode=h_mode)
    ...
    transfer_sources = matched[matched["yuklenme_orani_pct"] >= min_source_load_pct].copy()
    ...
    main_rename = {
        ...
        "karar_demand_kva": "Karar Demandı (kVA)",
        "algoritmik_pik_demand_kva": "Algoritmik Pik Demand (kVA)",
        "p95_demand_kva": "P95 Demand (kVA)",
    ...
    with tab_risk:
        ...
        st.info("Yüklenme Oranı = Karar Demandı / Trafo Gücü × 100. Karar Demandı = ilk algoritma mantığındaki Algoritmik Pik Demand.")
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Karar Demandı (kVA)": st.column_config.NumberColumn(format="%.2f"),
    ...
    with tab_transfer:
        ...
        if selectable.empty:
            st.success("Seçilen minimum yüklenme eşiğine göre aktarım önerisi üretilecek trafo bulunamadı.")
        else:
            selectable = selectable.sort_values("yuklenme_orani_pct", ascending=False)

            mahalle_sec = st.selectbox(
                "Mahalle seç",
                ["TÜMÜ"] + sorted([m for m in selectable["mahalle"].dropna().astype(str).unique() if m]),
            )

            if mahalle_sec != "TÜMÜ":
                selectable = selectable[selectable["mahalle"].eq(mahalle_sec)]
            ...
            source_capacity = float(selected_row["kurulu_guc_kva"])
            source_current = float(selected_row["karar_demand_kva"])
            source_target_demand = source_capacity * source_target_pct / 100
            needed_transfer = max(0.0, source_current - source_target_demand)

            c1, c2, c3, c4 = st.columns(4)
            c1.metric("Riskli Trafo", f"{selected_row['montaj_yeri']} / {selected_row['tr_code']}")
            c2.metric("Mahalle", selected_row["mahalle"])
            c3.metric("Mevcut Yüklenme", fmt(selected_row["yuklenme_orani_pct"], 1, "%"))
            c4.metric("Hedef İçin Aktarılacak Yük", fmt(needed_transfer, 2, " kVA"))
            ...
            candidates = recommend_transfer_candidates(
                ...
                candidate_upper_pct=candidate_upper_pct,
            ...
            if candidates.empty:
                ...
                candidate_cols = [
                    ...
                    "karar_demand_kva",
                    "yuklenme_orani_pct",
                    "aday_alabilecegi_yuk_kva",
                    "onerilen_aktarim_kva",
                    "kaynak_aktarim_sonrasi_yuklenme_pct",
                ...
                candidate_rename = {
                    ...
                    "tr_code": "Aday Trafo",
                    "mahalle": "Mahalle",
                    "kurulu_guc_kva": "Aday Trafo Gücü (kVA)",
                    "karar_demand_kva": "Aday Mevcut Demand (kVA)",
                    "yuklenme_orani_pct": "Aday Mevcut Yüklenme (%)",
                    "aday_alabilecegi_yuk_kva": "Aday Alabileceği Yük (kVA)",
                    "onerilen_aktarim_kva": "Önerilen Aktarım (kVA)",
                    "kaynak_aktarim_sonrasi_yuklenme_pct": "Riskli Trafo Aktarım Sonrası (%)",
                    "aktarim_sonrasi_demand_kva": "Aday Aktarım Sonrası Demand (kVA)",
                    "aktarim_sonrasi_yuklenme_pct": "Aday Aktarım Sonrası Yüklenme (%)",
                    "aktarim_sonrasi_risk": "Aday Aktarım Sonrası Risk",
                }
    ...
    with tab_detail:
        ...
        if matched.empty:
            st.warning("Eşleşen trafo yok. Veri Kontrol sekmesini kontrol et.")
        else:
            ...
            d2.metric("Karar Demandı", fmt(row["karar_demand_kva"], 2, " kVA"))
    ...
    with tab_quality:
        ...
        with st.expander("Eşleşme, filtre ve algoritma mantığı"):
            st.write(
                ...
                "Karar Demandı, kısa pencere skoruyla seçilen Algoritmik Pik Demand değeridir."
            )
